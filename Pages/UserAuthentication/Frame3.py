...
class Frame3(tk.Frame):
	def __init__(self, master, *args, **kwargs):
		...
		self.login = tk.Button(
			...
			activebackground='#121212',
			# image=self.btnimg,
			text='Login',  # add text since no image
	...
	def loginNow(self):
		...
		user_object = verify_user(email, password)
		if user_object:
			self.result['text'] = "Login successful! Please wait..."
			
			# Save user_id to file for auto-login
			with open('user', 'w') as f:
				f.write(str(user_object['user_id']))
			
			# Login to main app
			self.master.login(user_object)
		else:
			self.result['text'] = "Invalid email or password"
